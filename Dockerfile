...
RUN chmod go= /initdb \
 ...
 && rm -rf "$downloadDir" \
 && apk add --no-cache --virtual .build-deps python-dev python3-dev bison coreutils dpkg-dev dpkg flex gcc libc-dev libedit-dev libxml2-dev libxslt-dev make libressl-dev perl-utils perl-ipc-run util-linux-dev zlib-dev openldap-dev \
